...
class SILGCN(object):
    def __init__(self, data_config, pretrain_data):
        # argument settings
        self.model_type = 'socialsilgn'
        self.adj_type = args.adj_type
        self.alg_type = 'socialsilgn'
        ...
        *********************************************************
        ...
        Different Convolutional Layers:
            1. silgcn: defined in 'Similar Interaction Light Graph Convolutional Networks';
            2. sociallgn: 
            3. lightgcn:  
            4. ngcf: defined in 'Neural Graph Collaborative Filtering', SIGIR2019;
        """
        if self.alg_type in ['socialsilgn']:
            self.ua_embeddings, self.ia_embeddings = self._create_socialSILGN_embed()
        elif self.alg_type in ['wslgcn']:
            self.ua_embeddings, self.ia_embeddings = self._create_WSLGCN_embed()
        elif self.alg_type in ['wilgcn']:
            self.ua_embeddings, self.ia_embeddings = self._create_wilgcn_embed()
        elif self.alg_type in ['silgcn']:
            self.ua_embeddings, self.ia_embeddings = self._create_silgcn_embed()
        elif self.alg_type in ['sociallgnfn']:
            self.ua_embeddings, self.ia_embeddings = self._create_sociallgnfn_embed()
        elif self.alg_type in ['sociallgn']:
            self.ua_embeddings, self.ia_embeddings = self._create_sociallgn_embed()
        elif self.alg_type in ['lightgcn']:
            self.ua_embeddings, self.ia_embeddings = self._create_lightgcn_embed()
        elif self.alg_type in ['wigcn']:
            self.ua_embeddings, self.ia_embeddings = self._create_tincf_embed()
    ...
    def _split_WI_hat(self, X):
        SI_fold_hat = []
        fold_len = (self.n_items + self.n_items) // self.n_fold
        for i_fold in range(self.n_fold):
            start = i_fold * fold_len
            if i_fold == self.n_fold - 1:
                end = self.n_items + self.n_items
            else:
                end = (i_fold + 1) * fold_len
            H = X[start:end]
            SI_fold_hat.append(self._convert_sp_mat_to_sp_tensor(H))
    ...
    def _create_WSLGCN_embed(self):
        ...
        Item_fold_hat = self._split_WI_hat(self.similar_items_adj)
        ...
        for k in range(0, self.n_layers):
            ...
            temp_embed_social_users = []
            for fold in range(self.n_fold):
                temp_embed_social_users.append(tf.sparse_tensor_dense_matmul(Social_fold_hat[fold], users_embed))
            users_embed_social = tf.concat(temp_embed_social_users, axis=0)

            temp_embed_similar_items = []
            for fold in range(self.n_fold):
                temp_embed_similar_items.append(tf.sparse_tensor_dense_matmul(Item_fold_hat[fold], items_embed))
            items_embed_similar = tf.concat(temp_embed_similar_items, axis=0)

            users_embed_interaction, items_embed_interaction = tf.split(all_embed_interaction,
                                                                        [self.n_users, self.n_items],
                                                                        0)

            fusion_embed_users = [users_embed_interaction, users_embed_similar, users_embed_social]
    ...
    # combine both social and user's behavior similarity
    def _create_socialSILGN_embed(self):
        if self.node_dropout_flag:
            A_fold_hat = self._split_A_hat_node_dropout(self.norm_adj)
        else:
            A_fold_hat = self._split_A_hat(self.norm_adj)
    ...
    def _create_silgcn_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            fusion_embed_users = [users_embed_interaction, users_embed_similar]
            fusion_embed_users = tf.stack(fusion_embed_users, 1)
            users_embed_next = tf.reduce_mean(fusion_embed_users, axis=1, keepdims=False)
    ...
    def _create_sociallgnfn_embed(self):
        if self.node_dropout_flag:
            A_fold_hat = self._split_A_hat_node_dropout(self.norm_adj)
        else:
            A_fold_hat = self._split_A_hat(self.norm_adj)

        SI_fold_hat = self._split_SI_hat(self.social_adj)
        ...
        for k in range(0, self.n_layers):
            ...
            for f in range(self.n_fold):
                temp_embed_interaction.append(tf.sparse_tensor_dense_matmul(A_fold_hat[f], ego_embed))
            all_embed_interaction = tf.concat(temp_embed_interaction, axis=0)

            temp_embed_similar_users = []
            for f in range(self.n_fold):
                temp_embed_similar_users.append(tf.sparse_tensor_dense_matmul(SI_fold_hat[f], users_embed))
            users_embed_similar = tf.concat(temp_embed_similar_users, axis=0)
    ...
    def _create_sociallgn_embed(self):
        if self.node_dropout_flag:
            A_fold_hat = self._split_A_hat_node_dropout(self.norm_adj)
        else:
            A_fold_hat = self._split_A_hat(self.norm_adj)
        ...
        for k in range(0, self.n_layers):
            ...
            users_embed_interaction, items_embed_next = tf.split(all_embed_interaction, [self.n_users, self.n_items],
                                                                 0)
            users_embed_similar = tf.nn.tanh(tf.matmul(
                users_embed_similar, self.weights['W_2_%d' % k]) + self.weights['b_2_%d' % k])

            users_embed_interaction = tf.nn.tanh(
                tf.matmul(users_embed_interaction, self.weights['W_1_%d' % k]) + self.weights['b_1_%d' % k])

            combine_users_embed = tf.concat([users_embed_similar, users_embed_interaction], axis=1)

            users_embed_next = tf.nn.l2_normalize(
                tf.matmul(combine_users_embed, self.weights['W_3_%d' % k]) + self.weights['b_3_%d' % k])
        ...
        u_g_embeddings, i_g_embeddings = tf.split(all_embed, [self.n_users, self.n_items], 0)
        return u_g_embeddings, i_g_embeddings
    def _create_tincf_embed(self):
        if self.node_dropout_flag:
            A_fold_hat = self._split_A_hat_node_dropout(self.norm_adj)

        else:
            A_fold_hat = self._split_A_hat(self.norm_adj)
            B_fold_hat = self._split_A_hat(self.weight_adj)
        ...
        for k in range(0, self.n_layers):
            ...
            for f in range(self.n_fold):
                ...
                signal_embed.append(tf.sparse_tensor_dense_matmul(B_fold_hat[f], ego_embeddings))

            side_embeddings = tf.concat(temp_embed, 0)
            bi_embeddings = tf.concat(signal_embed, 0)
    ...
    def _create_lightgcn_embed(self):
        ...
        all_embeddings = tf.reduce_mean(all_embeddings, axis=1, keepdims=False)  # giá trị mean của 3 layer
        u_g_embeddings, i_g_embeddings = tf.split(all_embeddings, [self.n_users, self.n_items], 0)
        return u_g_embeddings, i_g_embeddings
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.pretrain == -1:
        ...
        pretrain_data = None
    model = SILGCN(data_config=config, pretrain_data=pretrain_data)
